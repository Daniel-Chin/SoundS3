...
from SoundS3.shared import HAS_CUDA
...
class Conv2dGruConv2d(nn.Module):
    def __init__(self, config):
        ...
        self.fc3 = nn.Sequential(
            ...
            nn.Linear(64, 256),
            nn.ReLU(),
            nn.Linear(256, CHANNELS[-1] * LAST_H * LAST_W)
        )
    ...
    def predict_with_symmetry(self, z_gt, sample_points, symm_func):
        z_SR_seq_batch = []
        hidden_r = torch.zeros([self.rnn_num_layers, z_gt.size(0), self.rnn_hidden_size])
        if HAS_CUDA:
            hidden_r = hidden_r.cuda()
